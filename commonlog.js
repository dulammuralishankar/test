// ─── Helper: safe Object copy (replaces Object.assign - not in Rhino ES5) ───
...
// ─── Helper: getElkFormattedTime (assumed existing, kept as-is) ──────────────
...
// FIX 1: null guard on api_base_path before split
...
if (query_string !== null) {
  query_string = query_string.length === 0 ? null : context.getVariable('request.header.request_querystring');
}

// FIX 2: null guard on log_level before toUpperCase
var log_level     = context.getVariable("logging_level") || "INFO";
var ceo_company_id = context.getVariable('request.header.ceo-company-id');

// FIX 3: custom_fields_log comes as JSON string — parse it safely
var custom_fields_log = [];
try {
  var rawCustomFields = context.getVariable("custom_fields_log");
  custom_fields_log   = rawCustomFields ? JSON.parse(rawCustomFields) : [];
  if (!Array.isArray(custom_fields_log)) {
    custom_fields_log = [];
  }
} catch (e) {
  custom_fields_log = [];
...
// ─── Helper: safe payload resolve ────────────────────────────────────────────
function resolvePayload() {
  var p = context.getVariable('api_mask_payload');
  return (p === "" || p === null || p === undefined) ? context.getVariable('request.content') : p;
}
...
function extractErrors() {
  ...
  if (statusCode && !statusCode.match(/2[0-9][0-9]/)) {
    // FIX 4: error_obj.equals → use === 
    ...
    if (error_obj && error_obj !== "null" && error_obj.trim() !== "") {
      try {
        ...
        for (var e in error_response_object.errors) {
          var error_message = (error_response_object.errors[e].description ||
                               error_response_object.errors[e].statusDescription);
          var error_code    = (error_response_object.errors[e].error_code ||
                               error_response_object.errors[e].statusCode);
...
// ─── Helper: process time calculation ────────────────────────────────────────
function calcProcessTimes() {
  ...
  return {
    req:   req_process_time,
    res:   res_process_time,
    total: apigee_total_process_time
  };
}

// ─── FIX 5: Read payload as ARRAY and loop, picking log_type from each object
var payloadRaw   = context.getVariable("payload");
var payloadArray = [];
try {
  var parsedPayload = payloadRaw ? JSON.parse(payloadRaw) : [];
  payloadArray      = Array.isArray(parsedPayload) ? parsedPayload : [parsedPayload];
} catch (e) {
  context.setVariable("debug.payload.parse.error", e.message);
}

// ─── Process each object in the array by its own log_type ────────────────────
for (var idx = 0; idx < payloadArray.length; idx++) {
  var currentObj = payloadArray[idx];

  // FIX 6: log_type from the JSON object itself, NOT from header
  ...
  if (log_type === 'request') {
    ...
    context.setVariable("log_message", "\n" + JSON.stringify(log_object));
    ...
    if (paa_logging_enabled !== "false") {
      ...
      context.setVariable("splunk_log_message", "\n" + JSON.stringify(paa_log_object));
    }
    ...
    var processTimes2 = calcProcessTimes();
    ...
    var errors2       = extractErrors();
    var processTimes3 = calcProcessTimes();
